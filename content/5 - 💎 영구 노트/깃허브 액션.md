...
- 수동 배포 번거로움
- 반복적 테스트 작업
- 기존 CI/CD 도구 복잡함과 진입 장벽 -> 데브옵스 엔지니어 반필수
솔직히 깃허브 쓰면 그냥 깃허브 액션 쓰는게 훨씬 쉽고 정신건강에 좋다.
## 핵심 구조
이벤트 -> 워크 플로우 ->  작업 ->  액션 -> 러너

**Workflow (워크플로우)** → **Job (작업)** → **Step (단계)** → **Action (액션)**
### 0. 특정 이벤트 발생
[[1 - 📚 참고 노트/ai 답변/GitHub Actions 완전 핸드북_ 초보자부터 중급자까지#이벤트와 트리거]]
발생한 이벤트가 워크플로우 
- push
- pull_request
- schedule
...
-  `uses`: 액션 사용
### 4. Action(액션)
재사용 가능한 코드 단위, 복잡한 작업 캡슐화
직접 만들기도 하고 이미 만들어진걸 가져다가 쓸수도 있음
깃허브 마켓 플레이스엔 2만개 이상의 액션 등록되어 있음
[[1 - 📚 참고 노트/ai 답변/GitHub Actions 완전 핸드북_ 초보자부터 중급자까지#기본 CI/CD 파이프라인]]
```
    steps:
    - name: 체크아웃
...
```
### 5. 러너 환경
깃허브에서 제공하는 가상 머신으로 유지보수 자동으로 처리
각 작업마다 가상머신 제공하고, 보통은 우분투로 쓴다.
월 2000분 무료, 퍼블릭은 무제한
원하면 직접 self-hosted 러너스 생성해서 만들수도 있음
...
## References
[[1 - 📚 참고 노트/ai 답변/GitHub Actions 완전 핸드북_ 초보자부터 중급자까지]]
[[1 - 📚 참고 노트/ai 답변/GitHub Actions 문법과 Next.js 실제 사용 사례 핸드북]]
 [[5 - 💎 영구 노트/CI CD]]
 [[5 - 💎 영구 노트/깃]]
 [[5 - 💎 영구 노트/젠킨스]]